export type ServeFeatures = {
  durationMs: number
  motionEnergy: number[] // normalized 0-1 per sampled frame
  centroidPath: { x: number; y: number }[] // normalized 0-1 coords
  peakTossIndex: number
  contactIndex: number
  followThroughIndex: number
  lowerBodyEngagement: number // 0-1
  shoulderRotationProxy: number // 0-1
  racquetDropProxy: number // 0-1
}

export type PlayerProfile = {
  name: "Nishikori" | "Federer" | "Djokovic" | "Alcaraz"
...
// Reference profiles approximated (heuristic numbers 0-1)
const PROFILES: PlayerProfile[] = [
  {
    name: "Nishikori",
    refs: {
      lowerBodyEngagement: 0.65,
      shoulderRotationProxy: 0.6,
      racquetDropProxy: 0.55,
    },
  },
  {
    name: "Federer",
    refs: {
      lowerBodyEngagement: 0.75,
      shoulderRotationProxy: 0.8,
      racquetDropProxy: 0.85,
    },
  },
  {
    name: "Djokovic",
    refs: {
      lowerBodyEngagement: 0.85,
      shoulderRotationProxy: 0.7,
      racquetDropProxy: 0.7,
    },
  },
  {
    name: "Alcaraz",
    refs: {
      // 若手トップ選手の特徴を仮置き（下半身駆動と回転強め）
      lowerBodyEngagement: 0.88,
      shoulderRotationProxy: 0.82,
      racquetDropProxy: 0.8,
    },
  },
]

// Extract a lightweight motion summary from a video element by sampling frames.
export async function extractServeFeatures(
  ...
  // read previous frame for difference
  ...
  for (let i = 0; i < sampleCount; i++) {
    ...
    for (let p = 0; p < data.length; p += 4) {
      ...
      // accumulate motion-weighted centroid
      const w = v
      diffSum += v
      mass += w
      cx += w * xCoord
    ...
    prev = new Uint8ClampedArray(data) // store copy
  }

  // Detect simple landmarks based on motion peaks
  const peakTossIndex = argMax(smooth(motionEnergy, 3))
  const contactIndex = Math.min(
    motionEnergy.length - 1,
    peakTossIndex + Math.max(1, Math.floor(sampleFps * 0.2))
  )
  const followThroughIndex = Math.min(
    motionEnergy.length - 1,
    contactIndex + Math.max(1, Math.floor(sampleFps * 0.3))
  )

  // Feature proxies
  const lowerHalfMotion = average(
    centroidPath.map((c, i) => (c.y > 0.6 ? motionEnergy[i] : 0))
  )
  const upperHalfMotion = average(
    centroidPath.map((c, i) => (c.y <= 0.6 ? motionEnergy[i] : 0))
  )
  const leftMotion = average(
    centroidPath.map((c, i) => (c.x < 0.5 ? motionEnergy[i] : 0))
  )
  const rightMotion = average(
    centroidPath.map((c, i) => (c.x >= 0.5 ? motionEnergy[i] : 0))
  )
...
export function compareToPros(features: ServeFeatures): SimilarityResult[] {
  return PROFILES.map((p) => {
    ...
    // Convert average delta (0-1) to score 0-100
...
export function generateAdvice(features: ServeFeatures, top: SimilarityResult) {
  ...
  // Helper to push advice when below reference
  const advise = (
    key: keyof ServeFeatures,
    label: string,
    how: string,
  ...
  advise(
    "lowerBodyEngagement",
    "Use legs more in the loading phase",
    "Deeper knee bend and drive up through your hips to transfer energy into the toss and upward jump. Try a slow-count load: 1-2 load, 3 explode.",
    1.2
  )

  advise(
    "shoulderRotationProxy",
    "Add more shoulder and trunk rotation",
    "Turn your hitting shoulder behind you in the trophy position and uncoil through contact. Keep non-dominant arm up longer to create stretch.",
    1.1
  )

  advise(
    "racquetDropProxy",
    "Increase racquet drop",
    "Relax your wrist and let the elbow lead so the racquet head drops behind your back before driving up (the 'scratch your back' feel).",
    1
  )
...
async function analyzeServeWithPython(video: HTMLVideoElement): Promise<AnalysisResult | null> {
  try {
    ...
    if (!clippedVideosData || !segmentsData) {
      throw new Error('クリッピング結果が見つかりません');
    }
    ...
    // 現在の動画のパスを使用
    const currentVideoPath = clippedVideos[0]?.path; // 最初のクリップを使用
    
    console.log('analyzeServeWithPython - clippedVideos:', clippedVideos);
    console.log('analyzeServeWithPython - segments:', segments);
    console.log('analyzeServeWithPython - currentVideoPath:', currentVideoPath);
    ...
    console.log('analyzeServeWithPython - 送信データ:', requestData);
    console.log('analyzeServeWithPython - JSON文字列:', JSON.stringify(requestData));
    ...
    try {
      if (result?.userCsv) {
        sessionStorage.setItem('poseAdviceUserCsv', result.userCsv);
      }
      if (result?.referenceSuggestions) {
        sessionStorage.setItem('poseAdviceReferenceSuggestions', JSON.stringify(result.referenceSuggestions));
      }
      if (result?.userClip) {
        sessionStorage.setItem('poseAdviceUserClip', JSON.stringify(result.userClip));
      }
    } catch (storeError) {
      console.warn('analyzeServeWithPython - failed to store pose advice data:', storeError);
    }
    
    if (result.success) {
      // モック結果をJavaScript形式に変換
      return {
...
async function videoToBase64(video: HTMLVideoElement): Promise<string> {
  return new Promise((resolve, reject) => {
    // 動画をキャンバスに描画してBase64に変換
    const canvas = document.createElement('canvas')
...
function convertMockFeatures(mockAnalysis: any): ServeFeatures {
  return {
    ...
    motionEnergy: [0.5, 0.6, 0.7, 0.8, 0.9, 0.8, 0.7, 0.6], // デフォルト値
    centroidPath: [
      { x: 0.5, y: 0.5 },
      { x: 0.5, y: 0.5 },
      { x: 0.5, y: 0.5 },
...
export type SimilarityAPIResult = {
  players: string[];
  probabilities: Record<string, number>;
  top1: { player: string; score: number };
};
  ...
  return [
    {
      ...
      deltas: { lowerBodyEngagement: 0.1, shoulderRotationProxy: 0.05, racquetDropProxy: 0.08 }
    },
    {
      player: 'Federer',
      score: Math.min(100, baseScore + Math.floor(Math.random() * 10) - 5),
      deltas: { lowerBodyEngagement: 0.08, shoulderRotationProxy: 0.12, racquetDropProxy: 0.06 }
    },
    {
      player: 'Djokovic',
      score: Math.min(100, baseScore + Math.floor(Math.random() * 10) - 5),
      deltas: { lowerBodyEngagement: 0.06, shoulderRotationProxy: 0.09, racquetDropProxy: 0.10 }
    },
    {
      player: 'Alcaraz',
      score: Math.min(100, baseScore + Math.floor(Math.random() * 10) - 5),
      deltas: { lowerBodyEngagement: 0.07, shoulderRotationProxy: 0.11, racquetDropProxy: 0.09 }
    }
  ].sort((a, b) => b.score - a.score);
}
...
function smooth(arr: number[], w = 3) {
  const out: number[] = []
...
function ensureVideoReady(video: HTMLVideoElement) {
  return new Promise<void>((resolve) => {
    if (video.readyState >= 2 && video.duration) return resolve()
    const onLoaded = () => {
      cleanup()
      resolve()
    }
...
function seekVideo(video: HTMLVideoElement, time: number) {
  return new Promise<void>((resolve) => {
    ...
    video.currentTime = Math.min(Math.max(time, 0), Math.max(0.001, video.duration - 0.001))
  })
}
